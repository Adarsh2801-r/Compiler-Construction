	...
	while(getline(fin,line)){
		...
		while(line[idx]){
			if((line[idx]>='A'&&line[idx]<='Z')||(line[idx]>='a'&&line[idx]<='z')){
				...
				switch(line[idx]){
					case '%': skipComments(line,&idx,lc); break; // get comments
					case '\t':
					case ' ': idx++; break; // ignore whitespaces,tabs
					case '+':
					case '-':
					case '*':
					case '/': 
					switch(line[idx]){
		            case '+':token_Set.push_back(newtk(300,"+",lc));symbol_table[300] = "+";break;
		            case '-':token_Set.push_back(newtk(301,"-",lc));symbol_table[301] = "-";break;
		            case '*':token_Set.push_back(newtk(302,"*",lc));symbol_table[302] = "*";break;
		            case '/':token_Set.push_back(newtk(303,"/",lc));symbol_table[303] = "/";break;
	                }
	                idx++; 
					...
					// delimiters
					switch(line[idx]){
		            case '{': token_Set.push_back(newtk(400,"{",lc));symbol_table[400] = "{";break;
		            case '}': token_Set.push_back(newtk(401,"}",lc));symbol_table[401] = "}";break;
		            case '(': token_Set.push_back(newtk(402,"(",lc));symbol_table[402] = "(";break;
		            case ')': token_Set.push_back(newtk(403,")",lc));symbol_table[403] = ")";break;
		            case ',': token_Set.push_back(newtk(404,",",lc));symbol_table[404] = ",";break;
		            case '[': token_Set.push_back(newtk(405,"[",lc));symbol_table[405] = "[";break;
		            case ']': token_Set.push_back(newtk(406,"]",lc));symbol_table[406] = "]";break;
		            case ';': token_Set.push_back(newtk(200,";",lc));symbol_table[200] = ";";break;
